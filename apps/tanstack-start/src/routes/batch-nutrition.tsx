...
import { z } from "zod";
import { getBatchNutrition } from "../server-fns/batch-nutrition";
import type { RecipeNutrition } from "../types";

const batchNutritionSearchSchema = z.object({
  ids: z.string().optional(),
});

export const Route = createFileRoute("/batch-nutrition")({
  validateSearch: batchNutritionSearchSchema,
  loaderDeps: ({ search }) => ({ ids: search.ids }),
  loader: ({ deps }) => {
    const idsString = deps.ids ?? "1,2,3";
    return getBatchNutrition({ data: idsString });
  },
  component: BatchNutritionPage,
});

function BatchNutritionPage() {
  const nutritionData = Route.useLoaderData();

  const totals = nutritionData.recipes.reduce(
    (
      acc: { calories: number; protein: number; fat: number; carbs: number },
            ...
            <div className="overflow-x-auto">
              <table className="w-full rounded-lg border border-zinc-200 dark:border-zinc-700">
                ...
                <tbody className="divide-y divide-zinc-200 dark:divide-zinc-700">
                  {nutritionData.recipes.map((recipe: RecipeNutrition) => (
